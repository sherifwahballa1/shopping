...
getProducts = catchAsync(async (req, res) => {
    ...
    // validate products quentity to retrieve exists products
    let filteredProducts = Object.values(products).filter(product => { return product.quentity > 0 });

    if (filteredProducts.length === 0) return res.status(404).json({ message: "There are no products", error: "404: Not Found" });
